...
export const JourneyGraphic = ({ steps, className = "" }: Props) => {
  ...
  return (
    <div
      className={`relative mx-auto w-full min-h-[min(72vw,520px)] max-w-none overflow-visible lg:min-h-[480px] ${className}`}
    >
      <motion.div
        className="relative h-full w-full overflow-visible"
        initial={false}
        animate={reduce ? undefined : { rotate: [0, 0.6, 0] }}
        ...
        <ul className="pointer-events-none absolute inset-0 m-0 list-none p-0">
          {nodes.map(({ step, index, x, y }) => {
            ...
            return (
              <motion.li
                key={step.id}
                className="absolute inline-flex w-max max-w-[calc(100vw-1.5rem)] -translate-x-1/2 -translate-y-1/2 items-center gap-1.5 whitespace-nowrap rounded-full border border-dra-taupe-deep/12 bg-white/95 px-3 py-2 text-left text-[11px] font-medium leading-none text-dra-taupe-deep shadow-md sm:px-3.5 sm:py-2.5 sm:text-sm"
                style={{
                  left: `${px * 100}%`,
                  top: `${py * 100}%`,
                }}
